 ...
 */
export async function listMCPTools(mcpId: McpServerId): Promise<unknown> {
...
>(mcpId: McpServerId, toolName: string, args: TInput): Promise<TOutput> {
	const { url, id } = MCP_SERVERS[mcpId] || findServerByUrl(mcpId);
	return internalCallService(url, id, {
		jsonrpc: "2.0",
...
function findServerByUrl(expected: string) {
	...
	const items = Object.values(MCP_SERVERS) as {
		url: string;
		name: string;
		id: string;
	}[];
