...
- Countries States Cities Database is used as backend-only location validation
  data so fake or ambiguous city/region/country input does not silently search
  an unrelated place.
...
place-name/geopolitical changes. It validates places only; it does not verify
restaurant data.
