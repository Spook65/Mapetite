...
function buildReviews(restaurantName, rating, reviewCount) {
  ...
  return Array.from({ length: Math.min(3, templates.length) }, (_, index) => {
    ...
    isOpenNow:
      typeof props.open_now === "boolean"
        ...
        : typeof props.is_open_now === "boolean"
          ...
          : undefined,
    hours: openingHours,
    photoUrl: buildPhotoUrl(`${placeId}-${index}`),
    galleryImageUrls: buildGalleryImages(placeId),
    photoAttributions: [],
